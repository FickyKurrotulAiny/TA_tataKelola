...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form class="main-form row" action="{{ route('persediaan.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <!-- @method('PUT') -->
                ...
                <div class="mb-3 col-md-6 col-xl-4">
                    ...
                    <input type="text" name="harga_satuan" placeholder="Harga Satuan"
                        class="form-control @error('harga_satuan') is-invalid @enderror" value="{{ old('harga_satuan') }}">
                    @error('harga_satuan')
                        <span class="text-danger">{{ $message }}</span>
                    @enderror
                </div>
